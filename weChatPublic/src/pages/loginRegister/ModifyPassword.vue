<template>
  <div class="modify-psd">
    <div class="tips fontsize-28">请输入{{isPhoneNumber}}收到的短信验证码</div>
    <div class="modify-input">
      <van-cell-group>
        ...
        >
         ...
         <!--  <div slot="button">
            <van-button @click="getCodeBtnClick" :disabled="!show">
...
export default {
  data(){
    return {
      ...
      isPhoneNumber: this.$route.query.phoneNumber,
      show:true,
      count:60,
      timer: null,
    }
  ...
  props:{},
  created(){
    //this.isPhoneNumber = isPhoneNumber.replace(/^(\d{3})\d{4}(\d{4})$/, "$1****$2"); //方法一
    this.isPhoneNumber = this.isPhoneNumber.substr(0, 3) + '****' + this.isPhoneNumber.substr(7);   
  },
  components:{
